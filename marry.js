...
export class example extends plugin {
    constructor (){
        super({
            ...
            rule: [
                {
                    reg: "^(#|/)?(今日老婆|今日CP|娶CP|cp|娶cp|今日cp|CP|marry|娶群友|娶群员|娶老婆|结婚|今日老公)$",
                    fnc: '今日老婆',
                },
                {
                    reg: "^(#|/)?(休妻|悔婚|离婚|换老婆|换老公)$",
                    fnc: '悔婚',
                }
            ]
        })
    ...
    async 今日老婆(e) {
        ...
        if (!marrydata  || notMarryToday ) {
            ...
            replyMessage = `今天已经迎娶【${marrydata.lastmarry.nickname}】了喵~`, 
    ...
    async 悔婚(e) {
        ...
        if (!marrydata.isRemarry && isRemarry || marrydata.isRemarry && isRemarry) {
    ...
    async recp(e) {
        ...
        if (/^(#|\/)?(是的|是|愿意|继续|我愿意|当然)$/.test(messageText)) {
            ...
            let replyMessage = `和${cpdata.lastCP.nickname}续婚了喵~`;
...
async function handleMarryAndCP(isRemarry, e, date_time, randomWife, selfMember, redis) {
    ...
    let content = isRemarry ? `解怨释结，更莫相憎；一别两宽，各生欢喜喵~` : "";
    let replyMessage = `${randomWife.nickname}成为了你的新CP喵~`;
...
async function generateFortune(e,replyMessage,content,imageUrl) {
    ...
    e.reply([segment.at(e.user_id), `的魅力不够，这次没娶到谁喵~还请再试试看喵~`]);
    } finally {
    if (browser) {
    await browser.close();
    }
