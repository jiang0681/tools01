...
export class memes extends plugin {
  ...
  async memesHelp (e) {
    e.reply('【memes列表】：查看支持的memes列表\n【表情名称】：memes列表中的表情名称，根据提供的文字或图片制作表情包,可以@或发出图片后引用\n【随机meme】：随机制作一些表情包\n【meme搜索+关键词】：搜索表情包关键词\n【表情名称+详情】：查看该表情所支持的参数')
  }
  ...
  async memes (e) {
    ...
    if (texts.length < info.params.min_texts) {
      await e.reply(`字不够！要至少${info.params.min_texts}个用/隔开！示例:ph(表情名称) Hello/world`, true)
      return true
    }
    texts.forEach(t => {
